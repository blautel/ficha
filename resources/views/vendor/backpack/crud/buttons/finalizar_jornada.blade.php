@if (is_null($entry->final) && (backpack_user()->id == $entry->user_id))
    @if ($crud->hasAccess('update'))
        @if (!$crud->model->translationEnabled())

        <!-- Single edit button -->
        <a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}" class="btn btn-sm btn-link"><i class="las la-flag-checkered"></i> Finalizar jornada</a>
